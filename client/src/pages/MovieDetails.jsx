...
const MovieDetails = () => {
  ...
  const handleFavorite = async () => {
    try {
      // Add proper null check for user context
  ...
  useEffect(() => {
    if (user === null) {
      // User info still loading
      return <Loading />; // Or a simple spinner/message
    }
    getShow();
  }, [id]);

  return show ? (
    <div className="px-6 md:px-16 lg:px-40 xl:px-100 pt-30 md:pt-50">
      <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
        <img
          src={image_base_url + show.movie.poster_path}
          alt=""
          className="max-w-70 h-104 object-cover rounded-xl max-md:mx-auto"
        />
